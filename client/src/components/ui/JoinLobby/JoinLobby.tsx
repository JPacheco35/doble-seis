                ...
                <CornerCard style={{ padding: '24px' }} cornerSize={15}>
                    ...
                    <Stack gap="md">
                        <Text style={{ color: '#f4e8c1', fontSize: 13 }}>
                            In lobby <span style={{ fontFamily: 'Abril Fatface, serif', color: '#f4b942' }}>{currentLobby?.code}</span>
                            {' — '}{currentLobby?.name}
                        </Text>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1px 1fr', gap: '0 12px' }}>
                            <div className="team-column">
                                ...
                                {Array.from({ length: 2 - (currentLobby?.players?.filter((p: any) => p.team === 1).length ?? 0) }).map((_, i) => (
                                    <div key={`e1-${i}`} className="player-slot-empty" />
                                ))}
                            </div>
                            ...
                            <div className="team-column">
                                ...
                                {Array.from({ length: 2 - (currentLobby?.players?.filter((p: any) => p.team === 2).length ?? 0) }).map((_, i) => (
                                    <div key={`e2-${i}`} className="player-slot-empty" />
                                ))}
                            </div>
                        </div>
